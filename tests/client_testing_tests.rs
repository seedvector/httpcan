...
mod tests {
    ...
    async fn test_timeout_behavior() {
        ...
        let result = client
            .get_with_timeout("/delay/5", Duration::from_secs(1))
            .await;
    ...
    async fn test_successful_request() {
        ...
        let result = client
            .get_with_timeout("/delay/1", Duration::from_secs(5))
            .await;

        assert!(result.is_ok(), "Request should succeed");
        let response = result.unwrap();
        assert!(
            response.contains("args"),
            "Response should contain request info"
        );
    }
    ...
    async fn test_json_response() {
        ...
        let response = client
            .get_with_timeout("/json", Duration::from_secs(5))
            .await
            .unwrap();
        ...
        assert!(
            json.get("slideshow").is_some(),
            "JSON should contain slideshow"
        );
    }
