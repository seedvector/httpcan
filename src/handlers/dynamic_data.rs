...
pub async fn base64_handler(_req: HttpRequest, path: web::Path<String>) -> Result<HttpResponse> {
...
pub async fn range_handler(
    ...
    if first_byte_pos > last_byte_pos || first_byte_pos >= numbytes || last_byte_pos >= numbytes {
...
pub async fn delay_handler_get(
    ...
    let delay_seconds = path.into_inner().min(10.0); // Max 10 seconds
