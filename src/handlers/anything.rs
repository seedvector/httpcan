...
pub async fn anything_handler_get(
    req: HttpRequest,
...
pub async fn anything_handler(
    ...
    let request_info =
        process_request_payload(&req, payload, &config.exclude_headers, None).await?;
    Ok(HttpResponse::Ok().json(request_info))
}
...
pub async fn anything_with_param_handler(
    ...
    let request_info = process_request_payload(
        &req,
        payload,
